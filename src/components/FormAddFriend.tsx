          ...
          <div className='z-20 fixed-center translate-y-[-50%] bg-white w-screen max-w-[500px] rounded-[10px] p-[20px] space-y-3'>
            ...
            <form
              ...
              <input
                ...
                type='text'
                placeholder={
                  isVietnames ? `Nhập tên người dùng` : 'Enter username'
                }
                autoFocus
              />
              <button
            ...
            {matchedFriends &&
              ...
              matchedFriends.map(({ uid, displayName, photoURL, fieldId }) => {
                const isRequested = user && user.requests?.includes(uid);

                const isFriend = user && user.friends?.includes(uid);
